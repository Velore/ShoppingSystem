...
public class InputFormat {
    ...
    static {
        ...
        MAIN_VIEW.put("2.用户登录", "login [-u userId](必填) [-p password](必填)");
        ...
        USER_VIEW.put("4.删除[指定]超市,del后的参数允许输入多个,但只有最后一个作为marketId",
                "market del [marketId](必填)");
        USER_VIEW.put("5.进入[指定]超市", "into [marketId](必填)");

        USER_VIEW.put("\n商品", "--------------------------------------");
        USER_VIEW.put("a.查询[多个]商品(默认查询全部商品,可限定商品名)", "prod [productName]");
        USER_VIEW.put("b.新增商品(root用户)", "prod ins [-n productName](必填) [del description]");
        USER_VIEW.put("c.修改[指定]商品信息(root用户)",
                "prod alter [-i productId](必填) [-n productName] [del description]");
        USER_VIEW.put("d.删除[指定]商品(root用户),del后的参数允许输入多个,但只有最后一个作为productId",
                "prod del [productId](必填)");
        ...
        MARKET_VIEW.put("2.新增商品(root用户)", "prod ins [-n productName](必填) [del description]");
        MARKET_VIEW.put("3.修改[指定]商品信息(root用户)",
                "prod alter [-i productId](必填) [-n productName] [del description]");
        MARKET_VIEW.put("4.删除[指定]商品(root用户)", "prod del [productId](必填)");
        ...
        MARKET_VIEW.put("b.删除[指定]库存,del后的参数允许输入多个,但只有最后一个作为productId",
                "store del [productId](必填)");
        ...
        MARKET_VIEW.put("3.检查[当前]超市订单,更新订单状态,若库存足够则返回[订单完成],否则返回[订单失败]",
                "order check [orderId]");
        MARKET_VIEW.put("删除[当前]超市的订单,del后参数可以有多个,但是只会删除最后一个", "order del [orderId]");
