...
public class InputFormat {
    ...
    static {
        ...
        MARKET_VIEW.put("b.删除[指定]库存", "store -d [-p productId](必填)");
        MARKET_VIEW.put("c.查询[多个]库存,默认[当前]超市全部库存,可限定商品", "store [-p product] [-min 最小库存数] [-max 最大库存数]");

        MARKET_VIEW.put("\n订单", "--------------------------------------");
        MARKET_VIEW.put("1.查询[当前]超市订单,可限定商品,订购数量和订单状态", "order [-p productId] [-min 最小订购数] [-max 最大订购数] [-s orderStatus]");
        MARKET_VIEW.put("2.新增[当前]超市订单", "order ins [-p productId](必填) [-n orderNum](必填)");
        MARKET_VIEW.put("3.检查[当前]超市订单,更新订单状态,若库存足够则[订单完成],否则[订单失败]", "order check [orderId]");

        MARKET_VIEW.put("获取帮助","help");
